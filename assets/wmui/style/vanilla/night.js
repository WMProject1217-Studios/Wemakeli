//By WMProject1217
console.log(`WMUI Style - vanilla
By WMProject1217`);
var bottombaruserlsn = 0;
var notify = {
    ...
    success: function (title, content, duration) {
        this.show(title, content, duration, 'background: #035F07;');
    },
    warning: function (title, content, duration) {
        this.show(title, content, duration, 'background: #620500;');
    },
    error: function (title, content, duration) {
        this.show(title, content, duration, 'background: #9A0000;');
    }
}

var wmuidialog = {
    show: function (title, content) {
        let fe  = $(`<div id="wmuidialogarea" style="display:none;">
        <div id="wmuidialog-face"></div>
            <div id="wmuidialog">
                <div id="wmuidialog-wrapper">
                    ...
                    <div class="wmuicenterutil" style='position:relative;top:-32px;'>
                        <button class="wmuibutton" onclick="wmuitoggledialog(false);">关闭</button>
                    </div>
                </div>
            </div>
        </div>
        `);
        $('#wmuidialog-container').append(fe);
        wmuitoggledialog(true);
    },
    delete: function() {
        document.getElementById("wmuidialog-container").innerHTML = "";
    }
}
