...
;
(function ($) {
    var danmaku = function (element, options) {
        this.$element = $(element);
        this.options = options;
        this.id = $(element).attr("id");
        $(element).data("nowTime", 0);
        $(element).data("danmakuList", options.danmakuList);
        ...
        this.$timer.timer({
            ...
            callback: function (index) {
                setTimeout(function () {
                    //计时前置  试验表明前置很好
                    if (me.options.danmakuLoop && $(element).data("nowTime") >= $(element).data("sumTime")) {
                        $(element).data("nowTime", 0);
                    }
                    ...
                    if ($(element).data("danmakuList")[$(element).data("nowTime")] && me.launched.indexOf($(element).data("nowTime")) < 0) {
                        var nowTime = $(element).data("nowTime");
                        var danmakus = $(element).data("danmakuList")[nowTime];
                        for (var i = (danmakus.length - 1); i >= 0; i--) {
                            setTimeout(me.checkRow(me), 0);
                            //setTimeout(me.rundanmaku(danmakus[i],nowCount,maxCount,nowSecCount,maxCountPerSec,options,me,$(element),speed,$(this)),1);
                            //  setTimeout(me.rundanmaku(danmakus[i],options,me,$(element),speed,$(this)),1);

                            // console.log(nowCount);
                            var a_danmaku = "<span class='danmaku' id='" + me.id + "tempDanmaku'></span>";
                            $(element).append(a_danmaku);
                            ...
                            if (danmaku.position == 0) {
                                ...
                                $("#" + topTmpId).fadeTo(options.topBottomdanmakuTime, $(element).data("opacity"), function () {
                                ...
                                $("#" + bottomTmpId).fadeTo(options.topBottomdanmakuTime, $(element).data("opacity"), function () {
                        ...
                        $(element).data("danmakuList")[nowTime] = danmakus
                    }  //if (danmakus)
    ...
    danmaku.prototype.danmakuStart = function () {
        this.$timer.timer('start');
        this.$element.data("paused", 0);
    };


    danmaku.prototype.danmakuStop = function () {
    ...
    danmaku.prototype.danmakuPause = function () {
    ...
    danmaku.prototype.danmakuResume = function () {
        this.$timer.timer('resume');
        $("#" + this.id + ' .danmaku').resume();
        this.$element.data("paused", 0);
    };

    danmaku.prototype.danmakuHideAll = function () {
        $("#" + this.id + ' .danmaku').css({"opacity": 0});
        this.initRows(this);
    };


    danmaku.prototype.setTime = function (arg) {
        $("#" + this.id + ' .danmaku').remove();
        this.$element.data("nowTime", arg);

    };

    danmaku.prototype.setOpacity = function (arg) {
    ...
    danmaku.prototype.adddanmaku = function (arg) {
        if (arg instanceof Array) {
            for (var i in arg) {
                if (this.$element.data("danmakuList")[arg[i]["time"]]) {
                    this.$element.data("danmakuList")[arg[i]["time"]].push(arg[i]);
                }
                else {
                    this.$element.data("danmakuList")[arg[i]["time"]] = [];
                    this.$element.data("danmakuList")[arg[i]["time"]].push(arg[i]);
                }
            }
        }
        else {
            if (this.$element.data("danmakuList")[arg.time]) {
                this.$element.data("danmakuList")[arg.time].push(arg);
            }
            else {
                this.$element.data("danmakuList")[arg.time] = [];
                this.$element.data("danmakuList")[arg.time].push(arg);
            }
        }
    };
    ...
    function Plugin(option, arg) {
        return this.each(function () {
            var $this = $(this);
            var options = $.extend({}, danmaku.DEFAULTS, typeof option == 'object' && option);
            var data = $this.data('danmaku');
            var action = typeof option == 'string' ? option : NaN;
            if (!data) $this.data('danmaku', (data = new danmaku(this, options)));
    ...
    $.fn.danmaku = Plugin;
    $.fn.danmaku.Constructor = danmaku;
